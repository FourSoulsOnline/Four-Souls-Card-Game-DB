...
export default class AccountsController {
  ...
  static async apiGetAccounts(req, res, next) {
    const accountsPerPage = req.query.accountsPerPage
      ? parseInt(req.query.accountsPerPage, 10)
      : 20;
  ...
  static async apiGetAccountByCredentials(req, res, next) {
    try {
      ...
      /* calling the DAO to retrieve the account by the username - D.D. */
      let account = await AccountsDAO.getAccountByCredentials(
        username,
        password
      );
      if (!account) {
  ...
  static async apiAddFriendToAccount(req, res, next) {
    try {
      ...
      if (accountResponse.modifiedCount === 0) {
        throw new Error("unable to add a friend");
      }
  ...
  static async apiDeleteAccount(req, res, next) {
    try {
      ...
      const accountResponse = await AccountsDAO.deleteAccount(accountId);
      res.json({ status: "success" });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
